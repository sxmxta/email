...
public class EmailSender {
	 ...
	 *            发送邮件的密码
	 */
	public EmailSender(final String smtpHostName, final String username, final String password) {
	...
	public void send(Email email) throws AddressException, MessagingException {
		...
		List<String> ccs = email.getSecondsToSend();
		if (ccs != null) {
			// 为每个邮件接收者创建一个地址
			Address[] ccAdresses = new InternetAddress[ccs.size()];
			for (int i = 0; i < ccs.size(); i++) {
				ccAdresses[i] = new InternetAddress(ccs.get(i));
			}
	 ...
	 * @throws MessagingException
	 */
	public void send(List<String> recipients, Email mail) throws AddressException, MessagingException {
		send(recipients, mail.getTitle(), mail.getContent());
	}
}
